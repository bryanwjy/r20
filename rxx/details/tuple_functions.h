...
constexpr auto transform(F&& callable, Tuple&& arg) {
    return [&]<size_t... Is>(__RXX index_sequence<Is...>) {
        ...
        return Result{std::invoke(
            callable, get_element<Is>(__RXX forward<Tuple>(arg)))...};
...
RXX_ATTRIBUTES(_HIDE_FROM_ABI, NODISCARD)
constexpr auto transform(F&& callable, Tuple1&& tuple1, Tuple2&& tuple2) {
    return [&]<size_t... Is>(__RXX index_sequence<Is...>) {
        using Result = tuple<std::invoke_result_t<F&,
            std::invoke_result_t<decltype(get_element<Is>), Tuple1>,
            std::invoke_result_t<decltype(get_element<Is>), Tuple2>>...>;

        return Result{std::invoke(callable,
            get_element<Is>(__RXX forward<Tuple1>(tuple1)),
            get_element<Is>(__RXX forward<Tuple2>(tuple2)))...};
