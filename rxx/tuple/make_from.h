...
constexpr T make_from_tuple(Tuple&& args) noexcept(
    details::nothrow_make_from_tuple<T, Tuple>) {

    return [&]<size_t... Is>(__RXX index_sequence<Is...>) -> T {
        return T(ranges::get_element<Is>(__RXX forward<Tuple>(args))...);
